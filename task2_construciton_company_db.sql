...
begin
   ...
   -- INSERT INTO LW1_client (name, phone, email) VALUES ('ABC Company2', '545-343-3333', 'contact@johnsonenterprises.com');
    INSERT INTO LW1_client (name, phone, email) VALUES ('Johnson Enterprises3', '131-999-3333', 'contact@johnsonenterprwises.com');
end;

INSERT ALL
    INTO LW1_team (id, name, members) VALUES (-1, 'Team Alpha', 5)
    INTO LW1_team (id, name, members) VALUES (-2, 'Team Bravo', 3)
    INTO LW1_team (id, name, members) VALUES (-3, 'Team Charlie', 4)
    INTO LW1_team (id, name, members) VALUES (-4, 'Team Delta', 6)
    INTO LW1_team (id, name, members) VALUES (-5, 'Team Echo', 4)
    INTO LW1_team (id, name, members) VALUES (-6, 'Team Foxtrot', 5)
    INTO LW1_team (id, name, members) VALUES (-7, 'Team Golf', 4)
    INTO LW1_team (id, name, members) VALUES (-8, 'Team Hotel', 2)
    INTO LW1_team (id, name, members) VALUES (-9, 'Team India', 3)
    INTO LW1_team (id, name, members) VALUES (-10, 'Team Juliet', 5)
    INTO LW1_team (id, name, members) VALUES (-11, 'Team Kilo', 3)
    INTO LW1_team (id, name, members) VALUES (-12, 'Team Lima', 4)
SELECT * FROM dual;
...
CREATE SYNONYM LW1_peepee FOR MY_USER.LW1_project;

ALTER TABLE LW1_project
    ADD description CLOB;

ALTER TABLE LW1_client
    ADD CONSTRAINT lw1_uni_client_name UNIQUE (name);

ALTER TABLE LW1_client
    ADD CONSTRAINT lw1_uni_client_name_len CHECK (LENGTH(name) > 3);

CREATE INDEX LW1_client_name_idx ON LW1_client (name);

-- нельзя создавать проекты для одной команды с пересекающимися сроками
ALTER TABLE LW1_project ADD CONSTRAINT lw1_prj_period_overlap CHECK (
    SELECT EXISTS(
        SELECT 1 FROM LW1_project prj
        WHERE team_id = prj.team_id AND start_date <= prj."DATE" AND prj.start_date <= date1)
    FROM dual;
);

-- начало не может быть раньше конца
ALTER TABLE LW1_project ADD CONSTRAINT lw1_prj_period CHECK (start_date < date1);

-- в команде должно быть не меньше 3 человек
ALTER TABLE LW1_team ADD CONSTRAINT lw1_team_amount CHECK (members > 2);
